...
BEGIN
    ...
    FROM
        news_items ni
    WHERE
        ni.is_priority = TRUE AND ni.is_featured = TRUE AND ni.date <= NOW() AND ni.is_active = TRUE
        AND (search_tsquery IS NULL OR ni.fts_document @@ search_tsquery)
    ORDER BY
        CASE WHEN search_tsquery IS NOT NULL THEN ts_rank(ni.fts_document, search_tsquery) ELSE 1 END DESC, -- Rank by relevance
        ni.date DESC -- Fallback for tie-breaking or no search query
    OFFSET _offset
    LIMIT _limit;
END;
$$;
...
BEGIN
    ...
    ORDER BY
        CASE WHEN search_tsquery IS NOT NULL THEN ts_rank(r.fts_document, search_tsquery) ELSE 1 END DESC, -- Rank by relevance
        r.date_approved DESC -- Fallback for tie-breaking or no search query
    OFFSET _offset
    LIMIT _limit;
END;
$$;
